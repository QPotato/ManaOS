...
all: 
	...
	cd threads; $(MAKE) ManaOS
	cd userprog; $(MAKE) depend 
	cd userprog; $(MAKE) ManaOS 
	cd vm; $(MAKE) depend
	cd vm; $(MAKE) ManaOS 
	cd filesys; $(MAKE) depend
	cd filesys; $(MAKE) ManaOS 
	cd network; $(MAKE) depend
	cd network; $(MAKE) ManaOS 
	#cd bin; make all
...
clean:
	$(SH) -c "rm -f */{core,ManaOS,DISK,*.o,swtch.s} test/*.coff bin/{coff2flat,coff2noff,disassemble,out}"
