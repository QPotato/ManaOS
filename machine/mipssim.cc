...
void Machine::OneInstruction(Instruction *instr)
{
	...
	case OP_SB:
		if (!machine->WriteMem((unsigned)(registers[(int)instr->rs] + instr->extra), 1, registers[(int)instr->rt]))
			return;
		break;

	case OP_SH:
		if (!machine->WriteMem((unsigned)(registers[(int)instr->rs] + instr->extra), 2, registers[(int)instr->rt]))
			return;
		break;
	...
	case OP_SLLV:
		registers[(int)instr->rd] = registers[(int)instr->rt] << (registers[(int)instr->rs] & 0x1f);
		break;
	...
	case OP_SW:
		if (!machine->WriteMem((unsigned)(registers[(int)instr->rs] + instr->extra), 4, registers[(int)instr->rt]))
			return;
		break;
